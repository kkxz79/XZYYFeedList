...
- (instancetype)init {
    ...
    @weakify(self);
    self.touchBlock = ^(XZControl *view, YYGestureRecognizerState state, NSSet *touches, UIEvent *event) {
        @strongify(self);
        if (!self) return;
    ...
    self.longPressBlock = ^(XZControl *view, CGPoint point) {
        @strongify(self);
        if (!self) return;
...
- (instancetype)init {
    ...
    _avatarView.touchBlock = ^(XZControl *view, YYGestureRecognizerState state, NSSet *touches, UIEvent *event) {
        @strongify(self);
        if (!self) return;
        if (state == YYGestureRecognizerStateBegan) {
